...
class View(Dispatcher):
     ...
     def ResetToMap(self,pref_width=-1,pref_height=-1):
          mapwidth=self.map.GetMaxWidth()
          mapheight=self.map.GetMaxHeight()
...
class RoomView(View):
     ...
     def DrawParticles(self,screen):
          for i in self.room.particle.particleList:
               oAlpha=i.color.a
               i.color.a=float(oAlpha)*(float(self.alpha)/float(255))
               shape=i.shape.Clone()
     ...
     def FocusTo(self,id,once=False):
          ...
          if obj != None:
               self.SetCamPosition(obj.posX-(self.width*TILEWIDTH/2)+(obj.width/2)+self.offFocusX, \
               obj.posY-(self.height*TILEHEIGHT/2)+(obj.height/2)+self.offFocusY)
               self.BroadcastMessage(ViewMessage(VIEW_FOCUS,self))
               
     def FocusToScroll(self,id,time):
          obj=self.room.GetObject(id)
          if obj != None:
     ...
     def FadeIn(self,time,end=255,start=0):
          ...
          self.fadeEnd=end
          self.fadeInterval=int(ceil(-1*((start-end)/(time))))
          self.fadeStep=0
          self.fadeTime=time
