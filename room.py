...
class Room(Dispatcher):
     ...
     def AddObject(self,otype,posx=0,posy=0,name=""):
          ...
          if self.GetController(obj.type.controller) != None and self.editor==False:
               ctrl=self.AddController(obj.type.controller,0,(self,obj))
     ...
     def MoveObject(self,obj,velx,vely,object=True):
          ...
          landed=False
          deaccel=False
          blockedX, blockedY = False, False
          passed=False
          
          if type(obj) == type(123):
               obj=self.GetObjectById(obj)
          
          if velx < 0:
               #if totally clear on the left, move accordingly
               if r["upLeft"] and r["centerLeft"] and r["downLeft"]:
                    #if obj moves to edge of room, hit wall
                    if obj.posX+velx < 0:
                         obj.posX=0
                         deaccel=True
                         wallthud=True
                    ...
                    obj.posX=int(obj.posX+velx) if obj.posX+velx>= 0 else 0
               #something is blocking the obj from the left
               else:
                    #obj hit a wall, deaccelerate
                    if r["obj"] == None:
                    ...
                    #obj hit another obj (both are not ghosts)
                    elif obj.collision != GHOST and r["obj"].collision != GHOST:
                         #col obj is solid, can't be pushed
                         if r["obj"].collision == SOLID:
                              thud=True
                              blockedX=True
                              deaccel=True
                              obj.posX=r["obj"].posX+r["obj"].width
                         #col obj is pushable, if not blocked move
                         elif r["obj"].collision == PUSHABLE:
                              #try to move col obj
                              moved=self.MoveObject(r["obj"].id,-((r["obj"].posX+r["obj"].width)-int(obj.posX+velx)),0)
                              #now move obj next to col obj (whether it was blocked or not)
                              obj.posX=r["obj"].posX+r["obj"].width
                              thud=True
                              #obj is blocked only if col obj is blocked
                              if moved[0] == False: 
                                   blockedX=True
                                   deaccel=True
                              
                    #obj or col obj was a ghost
                    else:
                         passed=True
                         #no wall blocks obj
                         ...
                         #there is a wall!
                              ...
                              newX=(r["leftX"]+1)*TILEWIDTH
                              obj.posX=newX if newX >= 0 else 0
                              deaccel=True
                              wallthud=True
                              blockedX=True
          #now do everything again, except in the opposite side...
          elif velx > 0:
               maxX=(self.map.width*TILEWIDTH)-obj.width
               #if totally clear on the right, move accordingly
               if r["upRight"] and r["centerRight"] and r["downRight"]:
                    #if obj moves to edge of room, hit wall
                    if obj.posX+velx > maxX:
                         obj.posX=maxX
                         deaccel=True
                         wallthud=True
                         blockedX=True
                         
                    obj.posX=int(obj.posX+velx) if obj.posX+velx <= maxX else maxX
               #something is blocking the obj from the right
               else:
                    #obj hit a wall, deaccelerate
                    if r["obj"] == None:
                         newX=(r["rightX"]-1)*TILEWIDTH+(TILEWIDTH-obj.width)
                         obj.posX=newX if newX <= maxX else maxX
                         deaccel=True
                         wallthud=True
                         blockedX=True
                    ...
                    #obj hit another obj (both are not ghosts)
                         ...
                         if r["obj"].collision == SOLID:
                              ...
                              blockedX=True
                              deaccel=True
                              obj.posX=r["obj"].posX-obj.width
                         #col obj is pushable, if not blocked move
                         elif r["obj"].collision == PUSHABLE:
                              #try to move col obj
                              moved=self.MoveObject(r["obj"].id,(obj.posX+obj.width+velx)-(r["obj"].posX),0)
                              #now move obj next to col obj (whether it was blocked or not)
                              obj.posX=r["obj"].posX-obj.width
                              thud=True
                              #obj is blocked only if col obj is blocked
                    ...
                    #obj or col obj was a ghost
                         ...
                         if r["upRightTile"] and r["centerRightTile"] and r["downRightTile"]:
                              ...
                              obj.posX=obj.posX+velx if obj.posX+velx <= maxX else maxX
                         #there is a wall!
                         else:
                              newX=(r["rightX"]-1)*TILEWIDTH+(TILEWIDTH-obj.width)
                              obj.posX=newX if newX <= maxX else maxX
          ...
          #if obj was blocked, bounce
          if deaccel: obj.velX=floor( float(-obj.velX)*float(obj.bounceX) )
          
          #set up for y
          deaccel=False
          
          if vely < 0:
               #totally clear going up
               if r["upLeft"] and r["upCenter"] and r["upRight"]:
                    #obj hit the top of the room
                    if obj.posY+vely < 0:
                         wallthud=True
                         blockedY=True
                         deaccel=True
                         
                    obj.posY=obj.posY+vely if obj.posY+vely >= 0 else 0
               #something is blocking the obj
               else:
                    #there is a wall
                    if r["obj"]==None:
                         obj.posY=(r["upY"]+1)*TILEHEIGHT
                         deaccel=True
                         wallthud=True
                         blockedY=True
                    #obj hit another obj (both are not ghosts)
                    elif obj.collision != GHOST and r["obj"].collision != GHOST:
                         #if obj is a platform, col obj is on it
                         if obj.group == PLATFORM:
                              r["obj"].platform=obj.id
                    
                         #col obj cannot be moved; obj is blocked
                         if r["obj"].collision == SOLID and obj.group != PLATFORM:
                              thud=True
                              blockedY=True
                              deaccel=True
                              obj.posY=r["obj"].posY+r["obj"].height
                         #col obj is pushable
                         elif r["obj"].collision == PUSHABLE or obj.group == PLATFORM:
                              #try to move col obj
                              moved=self.MoveObject(r["obj"].id,0,-((r["obj"].posY+r["obj"].height)-(obj.posY+vely)))
                              #now move obj next to col obj (whether it was blocked or not)
                              obj.posY=r["obj"].posY+r["obj"].height
                              thud=True
                              #obj is blocked only if col obj is blocked
                         ...
                         #there is a wall!
                              ...
                              wallthud=True
                              blockedY=True
          #exact same thing, except on the other side                    
          elif vely > 0:
               maxY=self.map.height*TILEHEIGHT-obj.height
               #totally clear going down
               if r["downLeft"] and r["downCenter"] and r["downRight"]:
                    #obj hit the bottom of the room
                    if obj.posY+vely > maxY:
                         landed=True
                         blockedY=True
                         deaccel=True
                         
                    obj.posY=obj.posY+vely if obj.posY+vely <= maxY else maxY
               #something is blocking the obj
               else:
                    #there is a wall
                    if r["obj"]==None:
                         obj.posY=(r["downY"]-1)*TILEHEIGHT+(TILEHEIGHT-obj.height)
                         deaccel=True
                         landed=True
                         blockedY=True
                    #obj hit another obj (both are not ghosts)
                    elif obj.collision != GHOST and r["obj"].collision != GHOST:
                         #if col obj is a platform, obj is on it
                         if r["obj"].group == PLATFORM:
                         ...
                         #col obj is pushable
                              ...
                              #try to move col obj
                              moved=self.MoveObject(r["obj"].id,0,(obj.posY+obj.height+vely)-r["obj"].posY)
                              #now move obj next to col obj (whether it was blocked or not)
                              obj.posY=r["obj"].posY-obj.height
                              thud=True
                              #obj is blocked only if col obj is blocked
          ...
          #if obj was blocked, bounce
          if deaccel: obj.velY=floor( float(-obj.velY)*float(obj.bounceY) )
          if obj.velY == 0.0 or blockedY:
               obj.jump=False
          
          #object bumped into another object
          if thud:
          ...
          if passed:
               if (obj.collision == GHOST and r["obj"] != GHOST) or (r["obj"].collision == GHOST and obj != GHOST):
               ...
               elif obj.collision == GHOST and r["obj"] == GHOST:
          ...
          #return whether obj was blocked 
          ...
          if velx < 0:
               if r["upLeft"] and r["centerLeft"] and r["downLeft"]:
                    ...
                    if r["obj"] == None:
                         ...
                         deaccel=True
                         wallthud=True
                    elif (r["obj"].collision == SOLID and obj.collision != GHOST) or (r["obj"].collision != GHOST and obj.collision == SOLID):
                         ...
                         if object2.group != PLATFORM:
                              object2.posX=int(SetBound(object1.posX+object1.width,0,self.map.width*TILEWIDTH-object2.width))
                              thud=True
                              deaccel=True
                         else:
                              object2.posX=int(SetBound(object2.posX+velx,0,(self.map.width*TILEWIDTH)-object2.width))
                              self.MoveObject(object1, -(object1.posX+object1.width-object2.posX), 0)
                              if object2.posX <= object1.posX+object1.width:
                                   object2.posX=object1.posX+object1.width
                                   
                    elif (r["obj"].collision == PUSHABLE  and obj.collision != GHOST) or (r["obj"].collision != GHOST and obj.collision == PUSHABLE):
                         object1, object2 = None, None
                         if r["obj"].collision == PUSHABLE:
                              object1=r["obj"]
                              object2=obj
                         else:
                              object1=obj
          ...
          if vely < 0:
               if r["upLeft"] and r["upCenter"] and r["upRight"]:
                    ...
                    if r["obj"] == None:
                         ...
                         deaccel=True
                         wallthud=True
                    elif r["obj"].collision != GHOST and obj.collision != GHOST:
                         if obj.group != PLATFORM:
                              thud=True
                              deaccel=True
               ...
               if r["downLeft"] and r["downCenter"] and r["downRight"]:
                    ...
                    if r["obj"] == None:
                         ...
                         if r["downLeftTile"] and r["downCenterTile"] and r["downRightTile"]:
                              ...
                              if obj.height < TILEHEIGHT:
                                   obj.posY=int(r["upY"]*TILEHEIGHT)+(TILEHEIGHT-obj.height)
                              else:
                                   obj.posY=int(r["upY"]*TILEHEIGHT)
                              deaccel=True
                              landed=True
     ...
     def CollisionData(self,id,velx,vely,col=True):
          ...
          if col:
               ...
               #iterate over collided objs; if there is more than one,
               #get the one with the SOLID or PUSHABLE collision
               for i in objclist:
                    if i.collision==SOLID or i.collision==PUSHABLE:
                         objc=i
                         break
               
               #if there isn't any, they're all ghosts
               if objc == None and len(objclist) > 0: objc=objclist[0]
     ...
     def LogicObject(self,obj):
               ...
               if obj.platform != None:
                    ...
                    if platform.collision != GHOST and platform.group == PLATFORM:
               ...
               if moved:
                    ...
                    if obj.group == PLAYER or obj.group == ENEMY:
                         if obj.action[DOWN]:
                              if r["obj"] != None:
     ...
     OBJFALL, OBJCOLLIDE, OBJFLOOR = 1, 2, 3
     def Fall(self,id):
          obj=self.GetObject(id)
          if obj.jump == False:
     ...
     def UpdateAlarms(self):
          ...
          for i in range(len(aList)):
               ...
               #if removed == False and alarm.remove:
                    #self.PushFreeAlarmId(aList[i].id)
                    #self.alarmList.remove(aList[i])
                    #removed=True
     ...
     def PlaySound(self,sound,x=-1,y=-1):
          if self.database.ItemExists(SOUND,sound):
               ...
               #set volume according to how far player is from sound
                    ...
                    vol=SetBound((400-sqrt((self.player.posX-x)**2+(self.player.posY-y)**2))/400,0.0,1.0)
